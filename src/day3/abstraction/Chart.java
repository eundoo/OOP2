...
public abstract class Chart {
	 ...
	 *  - 목적
	 *  	* 하위 클래스들이 구현할 메소드에 대한 설계이다.
	 *  	* 추상 메소드가 포함된 클래스를 상속받은 하위 클래스(구현클래스)
	 * 		  자신의 상황에 맞게 설계만 되어 있는 메소드를 구체적으로 구현해야 한다.
	 * 		* 추상메소드가 포함된 클래스를 상속받은 하위 클래스(구현클래스)는 해당 메소드를 반드시 재정의 해야한다.
	 * 		  (구현 글래스는 추상 클래스를 포함할 수 없기 때문)
	 * 		* 추상메소드가 포함된 클래스는 상속받은 하위 클래스(구현클래스)는 사용방법이 동일한 구현 메소드를 가지게 된다.
	 * 추상클래스
	 * 	- 불완전한 설계도이다.
