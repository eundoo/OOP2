...
public class AccountCopyApp {
	...
	public static void main(String[] args) {
		...
		arr[0] = new Account("홍길동", 100000, 1111);
		arr[1] = new SecretAccount("김유신", 200000, 1111, 1234);
		arr[2] = new Account("강감찬", 300000, 1111);

		for (Account item : arr) {
			//System.out.println(item);
			//System.out.println(item instanceof UnCopiable);
			if (!(item instanceof UnCopiable)) {
				System.out.println("["+item.getName()+"]객체를 복사합니다.");
			} else {
